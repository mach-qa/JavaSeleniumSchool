package automationSchool.lesson1.ch01_data_types;

public class Ex1_Numbers {
    public static void main(String[] args) {
        byte numberOfStates = 50;
        short numberOfCountries = 194;
        int equatorLengthInKm = 40075;
        long distanceToSunInMeters = 149600000000L;

        System.out.println(numberOfStates);
        System.out.println(numberOfCountries);
        System.out.println(equatorLengthInKm);
        System.out.println(distanceToSunInMeters);
        int d = 5;
        System.out.println(d = d + 7);
        System.out.println(d);

        float pi = 3.141592653562643f;
        double betterPi = 3.141592653562643d;

        System.out.println(pi);
        System.out.println(betterPi);
    }
}
